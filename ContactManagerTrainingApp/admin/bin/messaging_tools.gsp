...
uses java.net.URL
uses gw.webservice.ab.ab801.messagingtoolsapi.MessagingToolsAPI
...
config.ServerOverrideUrl = new URL(MessagingToolsArgs.Server + "/ws/gw/webservice/ab/ab801/MessagingToolsAPI").toURI()
...
if( MessagingToolsArgs.Resync ) {
  ...
  var abcontactId = MessagingToolsArgs.ABContact
  if( destination == null or abcontactId == null ) {
    print( "Please include a ABContact public id and destination id via the -contact and -destination options." )
    return
  }
  try {
    api.resyncABContact( destination, abcontactId )
    print( "ABContact (${abcontactId}) resynced against destination ${destination}" )
  } catch ( e ) {
    print( "ABContact resync failed: ${e.Message}" )
  }
}
